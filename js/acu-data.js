...
const ACU_NORMAL_SPEC = {
  ...
  // ── 指端（指尖正中，不是指甲角）──────────────────────
  //
  // ⭐ 2026-08-18 新增第三類 kind:"tip"。中衝原本沒列在這張表裡，落到預設的 "palm"，
  //    結果它那塊皮膚的朝向被當成「手掌平面」—— 手張開正對鏡頭時圓盤會畫成一個
  //    **平躺在指甲上的正圓**（實測：與合谷同樣是 9.0°、59×58px），等於跟使用者說
  //    「這塊皮膚正對著你、最可信」。**剛好講反了。**
  //
  //    中衝與上面那五個井穴（少商/商陽/少衝/少澤/關衝）不同：
  //      井穴在**指甲角**  → 皮膚朝向是手指的側向 → kind:"side"
  //      中衝在**指尖正中**（WHO：指端中央 = lm[12]）→ 皮膚朝向是**順著手指戳出去**
  //
  //    所以手張開時它本來就是切著看的，圓盤該被壓扁成一條橫過指尖的線 ——
  //    那條線的方向就是手指指向哪裡。這是事實，不是警告（見 computeAcuGate 的 tip 豁免）。
  中衝穴: { kind: "tip", axis: [11, 12] }, // 中指遠節骨軸，法向順著指尖方向
...
/**
 * ⭐ tip 類（指端穴）的圓盤要往指尖外推多少（寸）。**純視覺，不動穴道座標。**
 *
 * 問題：圓盤以穴道點為圓心，而中衝的點就在指尖（lm[12]）。圓盤半徑 0.42 寸 ≈ 7.4mm，
 * 直接畫在點上等於**往回蓋住整片指甲**，看起來像「穴道在指甲裡面」。
 *
 * 解法：把圓盤沿著手指骨軸往外推一小段，讓它落在**指尖的端面**上而不是壓在指甲上。
 * 推的方向就是 tip 的法向量，所以：
 *   手張開（指尖切著看）→ 法向量幾乎躺在畫面上 → 推得最多，圓盤明顯離開指甲
 *   指尖對著鏡頭       → 法向量幾乎沿著視線 → 投影趨近 0 → 幾乎不推，圓盤留在點上
 * **這是正投影自然的結果，不是特別寫的 if。**
 *
 * 0.23 寸 × CUN_MM(17.66) ≈ **4.1mm**。這個值是視覺調出來的，覺得還壓著指甲就調大。
 */
const TIP_DISC_OFFSET_CUN = 0.23;
...
//
// ⚠ 2026-08-12 起這個門檻只管 palm 類穴道。side 類（二間/後溪/陽谷…）的最佳視角
//   本來就是手刀（掌面偏離 ~90°），被這道全域門檻擋掉等於誤殺它們最準的那批幀。
//   逐穴道的角度上限見下面的 ACU_ANGLE_LIMIT。
const TILT_MAX_DEG = 25;

// ── 逐穴道角度上限（2026-08-12 新增）──────────────────────────────────────────
//
// 【這張表在管什麼】
// 每個穴道長在不同的皮膚面上，「手要擺多正」的標準本來就不該一樣：
//   ‧ palm 類（合谷、陽池…）長在手背/手心正面 → 手背正對鏡頭時最準
//   ‧ side 類（二間、後溪、陽谷…）長在手指/手掌「側緣」，像書脊那一面
//     → **手背正對鏡頭時它們反而是切著看、最不準；手刀姿勢才是它們的最佳視角**
// 所以這裡管的角度不是「手歪幾度」，而是「**這個穴道自己那塊皮膚**偏離鏡頭幾度」，
// 也就是 computeAcuConfidence(穴道名).angleDeg（0=該塊皮膚正對鏡頭、90=切著看）。
// 兩類穴道用同一個判斷式，因為法向量已經各自算過了（見 ACU_NORMAL_SPEC）。
//
// 【數字怎麼來的 —— 不是拍腦袋，是從各穴道自己的公式反推】
// 誤差來源：公式是「從 landmark 往某方向走 d 寸」，這個位移是在**畫面 2D 空間**走的，
// 皮膚一傾斜，真實位移在畫面上會被投影縮短成 d·cosθ，但公式照走 d → 差了 d·(1−cosθ)。
//   誤差(mm) ≈ d × CUN_MM × (1 − cosθ)  ≤ HIT_MM
//   → θ_max = acos( 1 − HIT_MM / (CUN_MM × d) )
// d = 該穴道公式裡「以寸為單位的偏移」的向量長度。純 lerp / 直接取 landmark 的穴道
// d = 0（兩端點都是 landmark，手一轉一起縮，位置自動跟著對）→ 對投影縮短免疫。
//
//   穴道    kind  d(寸)   反推θ   採用   d 的來源
//   合谷穴  palm  0.826   30.4°   30    HG_CUN
//   陽池穴  palm  0       —       60    YC_T 純 lerp（免疫）
//   陽溪穴  side  1.835   20.2°   20    YX_CUN ← 全表最嚴，偏移量最大
//   陽谷穴  side  1.177   25.3°   25    YG_U + YG_F
//   液門穴  palm  0.410   43.6°   44    YM_CUN
//   中渚穴  palm  0       —       60    ZZ_T 純 lerp（免疫）
//   小骨空  palm  0       —       60    lm18 直接
//   中魁穴  palm  0       —       60    lm10 直接
//   大骨空  palm  0       —       60    lm3 直接（DG_EXT=0）
//   八邪穴  palm  0.710   32.8°   33    BX_FWD + BX4_ULNAR
//   二間穴  side  0.476   40.4°   40    EJ_RAD
//   三間穴  side  0.680   33.5°   34    SJ3_CUN + SJ3_PROX
//   前谷穴  side  0.340   48.2°   48    QG_ULNAR
//   腕谷穴  side  0.750   31.9°   32    WG_ULNAR
//   後溪穴  side  0.473   40.5°   40    HX_ULNAR
//   中衝穴  palm  0       —       60    lm12 直接（ZC_EXT=0）
//   魚際穴  palm  0.226   60.1°   60    YJ_PERP
//   神門穴  palm  0.838   30.1°   30    SM_CUN
//   太淵穴  palm  1.070   26.6°   27    TY_CUN + TY_BCUN
//   四縫穴  palm  0       —       60    指間關節 landmark 直接
//   少商/商陽/少衝/少澤/關衝  side  0.427  42.7°  43   _PROX + _LAT
//   勞宮穴  palm  0       —       60    LG_T 純 lerp（免疫）
//
// 🚨 這個模型只算了「投影縮短」這一種誤差，讀數字前必須知道它**沒有**涵蓋什麼：
//   ① 沒有涵蓋 landmark 本身在大角度就變不準（大角度時 MediaPipe 的 z 是在猜，
//      2026-07-22 實測：全黑輸入照吐 21 點）。→ 所以 d=0 的穴道不是真的可以歪到 90°，
//      一律壓在 ACU_ANGLE_HARD_CAP。
//   ② **沒有涵蓋公式常數本身的誤差，而這件事更致命** —— 陽谷 12.97mm、腕谷 7.74mm
//      在完全正對時就已經超過 2mm 了。對這些穴道，2mm 的預算在 0° 就花光，
//      表上那個角度是「假設公式完美」才成立的上限，實際上任何角度都不夠。
//      **這張表是「角度還能再吃掉多少預算」，不是「到這個角度就會準」。**
//   ③ 這些值**全部未經實測驗證**。要驗證得畫「誤差 vs angleDeg」曲線
//      （palm/side 分開），那是 PROGRESS 待辦裡掛著的事，資料還沒收。
//
// 【怎麼改】直接改下面的數字就好，單位是度，語意是「這塊皮膚偏離鏡頭超過幾度就標不可信」。
// 值越小 = 越嚴格。改完請一併更新 記錄控制/mediapipe版本.md。
const ACU_ANGLE_LIMIT = {
  // ── 手背 palm 類 ──
  合谷穴: 30,
  陽池穴: 60,
  液門穴: 44,
...
// 全穴道共同上限：不管公式多不受投影縮短影響，皮膚斜到這個程度以上，
// landmark 本身就已經在猜了（見上面 ⚠①）。表裡沒列到的穴道也用這個值。
// ⚠ 推定值，非實測。參考點：CONF_MID=0.4 對應 66°，這裡取略保守的 60°。
const ACU_ANGLE_HARD_CAP = 60;

// 超過上限多少度以內算「邊緣」（黃字提醒），超過就是「明顯不可信」（紅字）。
// 分兩級是為了避免在門檻附近抖動時提示一直跳。
// ⚠️ 2026-08-18 起兩級都會被擋下不畫（見 ACU_GATE_BLOCK），margin 只再決定提示的措辭：
//    edge =「再轉一點就好」、bad =「角度太斜」。
const ACU_ANGLE_MARGIN_DEG = 15;

/**
 * ⭐ 角度閘門是「硬擋」還是「軟降級」（2026-08-18 用戶決定：改成硬擋）。
 *
 *   true  = 超過該穴道的 ACU_ANGLE_LIMIT 就**不畫點**，只出提示要使用者轉手。
 *   false = 舊行為（2026-08-12～08-18）：超標照樣畫點，只換色 + 出提示。
 *
 * 用戶原話：「為每一個穴道量身定制一個檢測區間，超過就不要讓他顯示然後做出提醒」。
 *
 * ⚠️ **這個改動的代價要知道**：上限是 2026-08-12 從公式偏移量反推的推定值，偏保守。
 *    2026-08-18 合谷穴 1378 幀實測（`記錄控制/角度收樣分析_20260818.md`）顯示
 *    抖動在 10–60° 之間其實相當平坦，60° 之後才明顯上升 —— 也就是說用現行的
 *    合谷 30° 硬擋，會擋掉一段實際上還堪用的角度，使用者會比較常看到「點不見了」。
 *    **要放寬得先有各穴道自己的收樣資料**（目前只有合谷一穴），不可憑感覺調。
 *
 * 留成常數而不是寫死，是為了能一行切回舊行為做對照。
 */
const ACU_GATE_BLOCK = true;

/**
 * ⭐ 「圓盤已經看不到」的判準：圓盤**短軸**小於這麼多像素就算看不到（2026-08-18）。
 *
 * 用戶原話：
 * > 「限制角度是圓盤看不到的情況下我不希望他還會亂跑，所以還看得到的時候、
 * >   或者圓盤不會偏移前用圓盤，不行后就用限制」
 *
 * 也就是兩層分工：
 *
 * | 狀況 | 誰負責 | 行為 |
 * |---|---|---|
 * | 圓盤看得到（還是個橢圓） | **圓盤自己** | 點照畫；圓盤被壓扁的程度就是回饋，使用者自己看得出要轉多少 |
 * | 圓盤已經扁成一條線 | **角度限制** | 硬擋，不畫點 —— 這時使用者已經沒有任何線索能自我修正了 |
 *
 * ⚠️ **所以擋的門檻不是 `ACU_ANGLE_LIMIT`。** 那個是給提示與顏色用的
 * （合谷 30° 的時候圓盤還是個很清楚的橢圓，擋掉太早，使用者只會覺得點一直不見）。
 *
 * ⭐ 為什麼用「像素」不用「角度」：圓盤短軸 = 2 × 半徑 × cos θ，而半徑跟著 cunPx 走。
 * 手離鏡頭越遠圓盤越小，同一個角度下越早變得看不清 —— 用像素判斷會自動跟著距離調整，
 * 訂死一個角度不會。
 *
 * 6px 的由來：⚠️ **目視訂的，未經實測**。參考點是穴道點本身的半徑就有 2–5px
 * （`acupointRadius`），圓盤短軸比點還細的時候就真的只是一條線了。
 * 典型 cunPx≈40 時這相當於約 **80°**；cunPx≈34（手比較遠）時約 **78°**。
 */
const DISC_MIN_MINOR_PX = 6;
