...
let flShowAux = true;   // 邊界實驗頁：輔助線預設開
...
function flStartCamera(video, ctx, canvas) {
  ...
  const cam = flCamera = new Camera(video, {
    ...
    onFrame: async () => {
      ...
      edgePerf.pose = edgePerf.pose ? edgePerf.pose * 0.9 + (performance.now() - tPose0) * 0.1 : performance.now() - tPose0;
  ...
  if (chosenSide) {
    ...
    if (stable !== "dorsal" || cached == null || postureFail) {
      ...
      if (flOffsetMode === "tip") {
        ...
        if (pref && L) {   // 邊界實驗頁：每幀重量，EMA 平滑，不鎖定
          ...
          const dtEdge = performance.now() - tEdge0;
          edgePerf.edge = edgePerf.edge ? edgePerf.edge * 0.9 + dtEdge * 0.1 : dtEdge;
          const ul = prof ? prof.slices.filter((x) => Math.abs(x.s) <= 0.5 && x.ul).map((x) => x.ul / L) : [];
          edgePerf.found = !!(prof && prof.tip && ul.length);
          if (edgePerf.found) {
            const side = ul.slice().sort((p, q) => p - q)[ul.length >> 1];
            pref.tipFrac = pref.tipFrac != null ? pref.tipFrac + (prof.tip / L - pref.tipFrac) * FL_EDGE_EMA : prof.tip / L;
            pref.sideFrac = pref.sideFrac != null ? pref.sideFrac + (side - pref.sideFrac) * FL_EDGE_EMA : side;
          }
        }
      ...
      } else if (false) {   // 邊界實驗頁：不鎖定，永遠走下面的即時分支
...
// 跟原頁差別：① 遮罩整片畫出來 ② 邊緣每幀即時量、不鎖定 ③ 右上角顯示每幀花費。
const edgePerf = { pose: 0, edge: 0, found: false };
let edgeMaskCanvas = null, edgeMaskCtx = null, edgeShowMask = true;
function edgeDrawMask(ctx, W, H) {
  const m = flLastPoseResults && flLastPoseResults.segmentationMask;
  if (edgeShowMask && m) {
    if (!edgeMaskCanvas) { edgeMaskCanvas = document.createElement("canvas"); edgeMaskCtx = edgeMaskCanvas.getContext("2d"); }
    if (edgeMaskCanvas.width !== W || edgeMaskCanvas.height !== H) { edgeMaskCanvas.width = W; edgeMaskCanvas.height = H; }
    const c = edgeMaskCtx;
    c.globalCompositeOperation = "copy";
    c.drawImage(m, 0, 0, W, H);
    c.globalCompositeOperation = "source-in";   // 只在遮罩範圍內上色
    c.fillStyle = "rgba(0,229,255,0.45)";
    c.fillRect(0, 0, W, H);
    ctx.drawImage(edgeMaskCanvas, 0, 0);
  }
  const el = document.getElementById("edge-perf");
  if (el) el.textContent =
    `FPS ${flFps ? flFps.toFixed(1) : "—"} ｜ Pose＋遮罩 ${edgePerf.pose.toFixed(1)} ms ｜ 邊界檢測 ${edgePerf.edge.toFixed(1)} ms ｜ 邊緣${edgePerf.found ? "找到" : "沒找到"}`;
}
