...
const FACE_DETAIL = {
  ...
  'GV26': {
    ...
    uses: [
      // func 留空是刻意的（UI 會跳過標題）：這一穴目前沒有症狀歸屬，
      // 不要自行補一個適應症回來。
      { func: '', press: '以指腹垂直輕柔按壓。',
        note: '又稱人中穴。' },
    ],
  },
  'CV24': {
...
/**
 * 臉部穴道的代表色（圖冊小格、療程清單的色點都用這一個）。
 *
 * ⭐ 與 App 的 `FaceData.hue()` **同一組數字**（按眼／眉／鼻／口／側臉分群）。
 *    兩端的圖冊要看起來是同一個產品，這組 base 值改了就要兩邊一起改。
 *
 * 🚨 不要拿 `acuColor()` 來用：那支查不到穴名時 `findIndex` 回 -1、hue 落到 34，
 *    **23 個臉部穴道會全部同一個土黃色**，而且跟手部的「其他」群撞色。
 */
const FACE_GROUP_HUE = { eye: 262, brow: 292, nose: 16, mouth: 336, side: 92 };

function faceColor(code) {
  const idx = Math.max(0, FACE_ACUPOINTS.findIndex(a => a.code === code));
  const base = FACE_GROUP_HUE[(FACE_BY_CODE[code] || {}).group] || 262;
...
//
// ⭐ 2026-09-20 整批重建，**唯一來源**是
//    `D:\穴道按摩\資料存放區\穴道資料庫_手臉合併_20260917_修訂版.xlsx`
//    的「病症分類_手臉合併」分頁（臉部那一欄），逐穴照抄、沒有自行增補。
//    用戶當天拍板「以 xlsx 為準」，所以舊表自己多掛的那些對應**刻意移除**：
//      · 緩解目痛   舊有絲竹空 TE23、球後 EX-HN7、太陽 EX-HN5 → 資料庫沒有
//      · 緩解頭痛   舊有陽白 GB14、攢竹 BL2、上關 GB3 → 資料庫沒有；資料庫的絲竹空舊表反而缺
//      · 緩解牙痛   舊有地倉 ST4、巨髎 ST3 → 資料庫沒有；資料庫的顴髎 SI18 舊表缺
//      · 改善失眠   舊有太陽 EX-HN5 → 資料庫只有印堂
//      · 緩解感冒症狀 舊掛 4 穴，但資料庫的感冒症狀**臉部欄是空的** → 整項移除
//    ⚠️ 連帶：球後 EX-HN7 現在沒有任何症狀歸屬，和 GV26／GV27 一樣只剩圖冊看得到。
//
// 還沒實作定位的也列，選穴頁會標成「準備中」而不是假裝有。
const FACE_SYMPTOM_MAP = {
  // 資料庫「目痛」的臉部欄 ＝ 承泣、四白、睛明、瞳子髎、攢竹、陽白、魚腰
  '緩解目痛':   ['ST1', 'ST2', 'BL1', 'GB1', 'BL2', 'GB14', 'EX-HN4'],
  '緩解頭痛':   ['TE23', 'EX-HN3', 'EX-HN5'],
  '緩解牙痛':   ['ST5', 'ST6', 'ST7', 'SI18'],
  '緩解耳鳴':   ['ST7', 'GB3'],
  '改善失眠':   ['EX-HN3'],

  // ── 臉部專屬四項（手部沒有對應穴道）──
  '美容':       ['ST1', 'ST3', 'GB1', 'TE23', 'GB14', 'CV24', 'ST5', 'ST6', 'ST7', 'SI18'],
  '鼻子不適':   ['ST3', 'EX-HN3', 'LI20', 'GV25'],
  '顏面神經麻痺': ['ST4', 'ST6', 'SI18'],
  '口腔衛生':   ['CV24'],

  // 🔴 2026-09-20：「昏迷急救」整項刪除，GV26（水溝穴）因此**沒有任何症狀歸屬**，
  //    推薦系統永遠選不到它，只剩圖冊看得到。這是刻意的，歸屬待用戶決定。
};
